...
public class School {
	...
	public ArrayList<String> _toString() {
		...
		school.add("School " + _name);
		
		for( int i=0; i<_houses.size(); i++) {
			
			 school.add("house " + (i+1) + " is " + _houses.get(i).getName()); 
		}
		if ( _houses.size() == 0)
			school.add("This school has no house");
		
		for( int i=0; i< _courses.size(); i++) {
			school.add("course " + (i+1) + " is " + _courses.get(i).getName()); 
		}
		if( _courses.size() == 0)
			school.add("This school has no course!");
		
		for ( int i=0; i< _students.size(); i++) {
			school.add("student " + (i+1) + " is " +  _students.get(i).getName());
		}
		if( _students.size() == 0)
			school.add("This school has no student!");
		
		for( int i=0; i< _professors.size(); i++) {
			school.add("professor " + (i+1) +" is " + _professors.get(i).getName());
		}
		if( _professors.size() == 0)
			school.add("This school has no professor!");
		
		school.add("location is " + _location);
